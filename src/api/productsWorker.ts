...
export const productsGet = createAsyncThunk(
  ...
  export function setLocalProductFavorite(product: IProduct[]) {
  ...
  export function getLocalProductFavorite(): IProduct[] {
    return JSON.parse(localStorage.getItem(lsPrdName) || "[]");
  }
  
  export function setLocalProductFavoriteSingle(product: IProduct) {
    if (!product.id || !product.favorite) return;
  
    const sendProduct: IProduct[] = getLocalProductFavorite();
